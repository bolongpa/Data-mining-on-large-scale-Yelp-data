    ...
    if len(arg) != 5:
        ...
        if case_num == '1':
            basketrdd = data.map(lambda x: [i for i in x.split(',')])\
                ...
                .reduceByKey(lambda x, y: x + y).map(lambda x: (x[0], list(set(x[1])))).values()
        elif case_num == '2':
            basketrdd = data.map(lambda x: [i for i in x.split(',')]) \
        ...
        if singletons:
            ...
            if son_candi_2:
                ...
                son_candi_more = basketrdd.mapPartitions(
                    lambda x: APriori_more(x, son_candi_2, ps)).distinct().collect()
                if son_candi_more:
